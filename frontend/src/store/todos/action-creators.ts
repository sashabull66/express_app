...
export const getTodos = (callback?: () => void) => async (dispatch: Dispatch) => {
    try {
...
export const markDone = (todo: ITodosResponse ,callback?: () => void) => async (dispatch: Dispatch) => {
    try {
        dispatch(fetchTodosStart());
...
export const removeTodo = (todoId: string ,callback?: () => void) => async (dispatch: Dispatch) => {
    try {
        dispatch(fetchTodosStart());
...
export const CreateTodo = (todo: Pick<ITodosResponse, 'done' & 'title' & 'description'> ,callback?: () => void) => async (dispatch: Dispatch) => {
    try {
        dispatch(fetchTodosStart());
